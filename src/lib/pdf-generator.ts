...
// Función para generar el PDF usando un template existente
export async function generatePDF(formData: FormData): Promise<Blob> {
  try {
    // Cargar el PDF existente desde la ubicación pública
    const existingPdfBytes = await fetch("/check.pdf").then((res) => {
      if (!res.ok) {
        throw new Error(`Error al cargar el PDF template: ${res.status} ${res.statusText}`)
      }
    ...
    const addText = (text: string, x: number, y: number, size = 10, font = helveticaFont) => {
    ...
    const addTextPage2 = (text: string, x: number, y: number, size = 10, font = helveticaFont) => {
      if (secondPage) {
    ...
    if (formData.tipoDocumento && tipoDocY[formData.tipoDocumento as keyof typeof tipoDocY]) {
      addText("X", 333, tipoDocY[formData.tipoDocumento as keyof typeof tipoDocY], 10, helveticaBold)
    }
    ...
    // Dividimos el texto de forma de pago en líneas específicas según el formato del PDF
    let formaPagoY = 260
