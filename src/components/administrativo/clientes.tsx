...
import { Pencil, Trash2, ChevronDown, Send, ChevronLeft, ChevronRight, Search } from 'lucide-react'
...
export function Clientes() {
  ...
  useEffect(() => {
    const nuevosFiltrados = clientes.filter(cliente => {
      ...
      return (
        ...
        (filtros.emprendimientos.length === 0 || cliente.emprendimientos.some(empId => filtros.emprendimientos.includes(empId.toString()))) &&
        (filtros.tipologias.length === 0 || cliente.tipologias.some(tipId => filtros.tipologias.includes(tipId.toString()))) &&
  ...
  const enviarPorWhatsApp = async () => {
    if (clienteSeleccionado) {
      const departamentosInfo = departamentosSeleccionados.map(id => {
  ...
  const getEmprendimientosNames = (empIds: number[]) => {
    return empIds.map(id => emprendimientos.find(emp => emp.id === id)?.nombre).filter(Boolean).join(", ")
  }

  const getTipologiasNames = (tipIds: number[]) => {
    return tipIds.map(id => tipologias.find(tip => tip.id === id)?.nombre).filter(Boolean).join(", ")
  }
    ...
    const checkNotifications = () => {
      ...
      clientes.forEach(cliente => {
        if (cliente.proximo_contacto && new Date(cliente.proximo_contacto) <= now) {
          alert(`Es hora de contactar a ${cliente.nombre} ${cliente.apellido}. Último contacto: ${cliente.ultimo_contacto ? new Date(cliente.ultimo_contacto).toLocaleDateString() : 'No registrado'}`)
        }
      })
    }
  ...
  const handleSelectChange = (value: string, field: keyof Cliente) => {
    if (modoEdicion && clienteEditando) {
      setClienteEditando({
        ...clienteEditando,
        [field]: field === 'agencia_inmobiliaria_id' ? parseInt(value) : value
      })
    } else {
      setNuevoCliente({
        ...nuevoCliente,
                    ...
                    {(modoEdicion ? clienteEditando?.tipo : nuevoCliente.tipo) === "Inmobiliaria" && (
                      <div className="grid items-center grid-cols-4 gap-4">
                        ...
                        <Select
                          ...
                          value={modoEdicion ? clienteEditando?.agencia_inmobiliaria_id?.toString() : nuevoCliente.agencia_inmobiliaria_id?.toString()}
                        >
                          <SelectTrigger className="col-span-3">
                          ...
                          <SelectContent>
                            {agenciasInmobiliarias.map((agencia) => (
                              <SelectItem key={agencia.id} value={agencia.id.toString()}>{agencia.nombre}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                          ...
                          emprendimientos.map((emprendimiento) => (
                            <div key={emprendimiento.id} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                checked={((modoEdicion ? clienteEditando?.emprendimientos : nuevoCliente.emprendimientos) || []).includes(emprendimiento.id)}
                                onCheckedChange={(checked) => handleMultiSelectChange(checked as boolean, emprendimiento.id, 'emprendimientos')}
                              />
                              <label
                          ...
                          tipologias.map((tipologia) => (
                            <div key={tipologia.id} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                checked={((modoEdicion ? clienteEditando?.tipologias : nuevoCliente.tipologias) || []).includes(tipologia.id)}
                                onCheckedChange={(checked) => handleMultiSelectChange(checked as boolean, tipologia.id, 'tipologias')}
                              />
                              <label
          ...
          <TableBody>
            {currentItems.map((cliente) => (
              <TableRow key={cliente.id}>
                ...
                <TableCell>{cliente.caracteristica}-{cliente.telefono}</TableCell>
                <TableCell>{cliente.email}</TableCell>
                <TableCell>{cliente.como_nos_conocio}</TableCell>
                <TableCell>{cliente.metros_min}-{cliente.metros_max}</TableCell>
                <TableCell>{cliente.precio_min}-{cliente.precio_max}</TableCell>
                <TableCell>{cliente.rango_edad}</TableCell>
                <TableCell>
                  <Badge variant={cliente.estado === 'Completo' ? 'default' : 'secondary'}>
                    {cliente.estado}
                  </Badge>
                </TableCell>
                <TableCell>{cliente.dato_extra}</TableCell>
                <TableCell>{cliente.ultimo_contacto ? new Date(cliente.ultimo_contacto).toLocaleDateString() : 'N/A'}</TableCell>
                <TableCell>{cliente.proximo_contacto ? new Date(cliente.proximo_contacto).toLocaleDateString() : 'N/A'}</TableCell>
                <TableCell>
      ...
      <Dialog open={dialogoEnvioAbierto} onOpenChange={setDialogoEnvioAbierto}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Selecciona los departamentos para enviar al cliente.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
