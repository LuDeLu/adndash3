...
const sampleParkingInfo = "Disponemos de 50 espacios de estacionamiento subterráneo. Costo adicional de $15,000 por unidad. Acceso con tarjeta magnética y vigilancia 24/7."


const samplePromotions = "10% de descuento en compras al contado. Financiamiento especial: 24 cuotas sin interés en unidades seleccionadas."

const sampleFloors: Floor[] = [
  { number: 1, availableUnits: 3, reservedUnits: 0, soldUnits: 0, path: "M446,697 L972,697 L972,726 L446,726", x: 448, y: 680 },
  { number: 2, availableUnits: 3, reservedUnits: 0, soldUnits: 0, path: "M448,629 L969,629 L969,675 L448,675", x: 448, y: 606 },
  { number: 3, availableUnits: 1, reservedUnits: 0, soldUnits: 2, path: "M971,606 L448,606 L448,560 L971,560", x: 448, y: 527 },
  { number: 4, availableUnits: 3, reservedUnits: 0, soldUnits: 0, path: "M972,538 L448,538 L448,493 L972,493", x: 448, y: 449 },
  { number: 5, availableUnits: 0, reservedUnits: 0, soldUnits: 3, path: "M972,469 L449,469 L449,425 L972,425", x: 448, y: 368 },
  { number: 6, availableUnits: 1, reservedUnits: 1, soldUnits: 1, path: "M450,403 L971,403 L971,357 L450,357", x: 448, y: 288 },
  { number: 7, availableUnits: 1, reservedUnits: 1, soldUnits: 1, path: "M973,335 L447,335 L447,283 L973,283", x: 448, y: 207 },
  { number: 8, availableUnits: 3, reservedUnits: 0, soldUnits: 0, path: "M971,262 L424,262 L424,210 L971,210", x: 420, y: 125 },
  { number: 9, availableUnits: 1, reservedUnits: 0, soldUnits: 1, path: "M913,199 L427,199 L427,148 L913,148", x: 420, y: 55 }
]
...
export function ProjectModal({ project, isOpen, onClose, onViewProject, onViewGallery, onViewPlanes }: ProjectModalProps) {
  ...
  const floorsToDisplay = project.floors || sampleFloors
  ...
  const getFilteredUnits = useCallback((floor: Floor) => {
                  ...
                  <g transform="scale(2.4, 2.4) translate(-515, 135)">
                    <AnimatePresence>
                      {floorsToDisplay.map((floor) => {
                        ...
                        const soldWidth = (floor.soldUnits / totalUnits) * floorWidth;
                        const reservedWidth = (floor.reservedUnits / totalUnits) * floorWidth;
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                <ScrollArea className="flex-1 p-4 sm:p-6">
                  ...
                  <div className="block sm:hidden mb-4">
                    <Select
                      onValueChange={(value: string) => setActiveTab(value as 'overview' | 'units' | 'features' | 'financial' | 'location')}
                      value={activeTab}
                    >
                  ...
                  <Tabs value={activeTab} onValueChange={(value: string) => setActiveTab(value as 'overview' | 'units' | 'features' | 'financial' | 'location')}>
                    <TabsList className="hidden sm:flex h-12 items-center justify-start rounded-lg bg-muted p-1 text-muted-foreground mb-4 overflow-x-auto whitespace-nowrap w-full">
                      ...
                      <TabsTrigger value="features" className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring f-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow">
                        Características
                      </TabsTrigger>
                      <TabsTrigger value="financial" className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring f-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow">
                        Financiamiento
                      </TabsTrigger>
                      <TabsTrigger value="location" className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring f-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow">
                        Ubicación
                      </TabsTrigger>
                    </TabsList>
                    <TabsContent value="overview" className="mt-2">
                      <div className="space-y-4 mt-4">
                        ...
                        <div className="space-y-2">
                          <h4 className="font-medium text-sm sm:text-base">Unidades disponibles: {project.available_units}</h4>
                          <Progress value={(project.available_units / project.total_units) * 100} className="w-full" />
                        </div>
                        <div className="space-y-2">
                          <h4 className="font-medium text-sm sm:text-base">Unidades reservadas: {project.reserved_units}</h4>
                            ...
                            {floorsToDisplay.map((floor) => (
                              ...
                              >
                                ...
                                <span className="text-[10px] sm:text-xs font-semibold mt-2">
                                  {getFilteredUnits(floor)} {currentFilter === 'all' ? 'Total' : currentFilter === 'available' ? 'Libre' : currentFilter === 'reserved' ? 'Reservadas' : 'Vendidas'}
                                </span>
                              </Button>
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="p-6 rounded-lg shadow-md">
                            ...
                            <ul className="space-y-2">
                              ...
                              <li><span className="font-semibold">Código postal:</span> C1425</li>
                            </ul>
                          </div>
                          <div className=" p-6 rounded-lg shadow-md">
                            <h5 className="font-medium text-lg mb-3">Comodidades</h5>
                            <p>
                              DOME Palermo Residence se encuentra ubicado en una importante esquina de Palermo, 
                              con amplia conexión con medios de transporte y en el centro de un importante polo 
                              gastronómico y recreativo de la ciudad.
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-2 sm:p-4 bg-background border-t">
                  <Button onClick={() => onViewPlanes(project.id)} className="w-full py-2 sm:py-6 text-xs sm:text-base">Ver planos</Button>
                  <Button 
                  ...
                  <Button onClick={() => onViewGallery(project.id)} className="w-full py-2 sm:py-6 text-xs sm:text-base">
