...
import { DomeProjectModal } from "./dome-project-modal"
import DomeFloorPlan from "./dome-floor-plan"
import DomeGallery from "./dome-gallery"
import { domeProjectInfo } from "@/lib/dome-puertos-data"
...
interface DomeProjectShowcaseProps {
  onBack?: () => void
}

export default function DomeProjectShowcase({ onBack }: DomeProjectShowcaseProps) {
  ...
  if (currentView === "floorPlan") {
    return <DomeFloorPlan floorNumber={selectedFloor} onReturnToProjectModal={handleReturnToProjectModal} />
  }

  if (currentView === "gallery") {
    return <DomeGallery onReturnToProject={handleReturnToCard} />
  }
        ...
        <div className="bg-card rounded-lg shadow-lg overflow-hidden cursor-pointer" onClick={handleViewProject}>
          <div className="relative h-64">
            ...
            <div className="absolute bottom-4 left-4 text-white">
              ...
              <h2 className="text-2xl font-bold">{domeProjectInfo.name}</h2>
              <p className="text-sm opacity-90">{domeProjectInfo.location}</p>
            </div>
          </div>
          <div className="p-6">
            <p className="text-muted-foreground mb-4">{domeProjectInfo.description}</p>
