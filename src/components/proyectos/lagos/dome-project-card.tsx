...
import { domeProjectInfo, getDomeProjectStats, formatPrice } from "@/lib/dome-puertos-data"
import Image from "next/image"

type DomeProjectCardProps = {
  onViewProject: () => void
  onViewFloorPlans: () => void
  onViewGallery: () => void
}

export default function DomeProjectCard({ onViewProject, onViewFloorPlans, onViewGallery }: DomeProjectCardProps) {
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 text-white">
          ...
          <p className="text-xl font-bold">{formatPrice(157400)}</p>
        </div>
      </div>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-2">
            ...
            />
            <div>
              <CardTitle className="text-lg font-bold text-gray-900">{domeProjectInfo.name}</CardTitle>
              <div className="flex items-center text-sm text-gray-600 mt-1">
                <MapPin className="h-4 w-4 mr-1" />
                {domeProjectInfo.location}
              </div>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        {/* Descripción */}
        <p className="text-sm text-gray-600 line-clamp-2">{domeProjectInfo.description}</p>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="font-medium">{domeProjectInfo.totalUnits}</p>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="font-medium">{domeProjectInfo.floors}</p>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            {domeProjectInfo.unitTypes.slice(0, 3).map((type, index) => (
              <Badge key={index} variant="outline" className="text-xs">
                {type.type}
              </Badge>
            ))}
            {domeProjectInfo.unitTypes.length > 3 && (
              <Badge variant="outline" className="text-xs">
                +{domeProjectInfo.unitTypes.length - 3} más
              </Badge>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1">
            {domeProjectInfo.amenities.slice(0, 3).map((amenity, index) => (
              <Badge key={index} variant="secondary" className="text-xs">
                {amenity.name}
              </Badge>
            ))}
            <Badge variant="secondary" className="text-xs">
              +{domeProjectInfo.amenities.length - 3} más
            </Badge>
          </div>
        </div>
        ...
        <div className="border-t pt-3 text-center">
          <p className="text-xs text-gray-500">{domeProjectInfo.generalPromotions}</p>
        </div>
      </CardContent>
    </Card>
  )
